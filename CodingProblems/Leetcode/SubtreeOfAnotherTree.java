 ...
 */
class Solution {
    //TC: O(n*m) SC: minimum between O(m) and O(n)
...
class Solution {
    public boolean isSubtree(TreeNode root, TreeNode subRoot) {
        ...
        if(treesAreSameHelper(root, subRoot)) return true;
        return isSubtree(root.left, subRoot) || isSubtree(root.right, subRoot);
    }
    
    public boolean treesAreSameHelper(TreeNode root, TreeNode subRoot) {
        ...
        return treesAreSameHelper(root.left, subRoot.left) && 
               treesAreSameHelper(root.right, subRoot.right);
    }
}