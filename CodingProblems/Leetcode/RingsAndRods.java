...
class Solution {
    public int countPoints(String rings) {
        ...
        for (int i = 0; i < rings.length(); i += 2) {
            ...
            int currNum = rods[rod];
            if (currNum == 0) {
                rods[rod] = colorNum;
            } else if (currNum == 1 && (colorNum == 2 || colorNum == 4)) {
                rods[rod] += colorNum;
            } else if (currNum == 2 && (colorNum == 1 || colorNum == 4)) {
                rods[rod] += colorNum;
            } else if (currNum == 4 && (colorNum == 1 || colorNum == 2)) {
                rods[rod] += colorNum;
            } else if (currNum == 5 && colorNum == 2 || currNum == 3 && colorNum == 4 || currNum == 6 && colorNum == 1) {
                count++;
                rods[rod] += colorNum;
            }
